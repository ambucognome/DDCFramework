...
public final class DropDownList: UIView {
	...
	public static weak var VisibleDropDown: DropDownList?
	...
	*/
	public var animationEntranceOptions: UIView.AnimationOptions = DropDownList.animationEntranceOptions
	
	/**
	The option of the hide animation. Only change the caller. To change all drop down's use the static var.
	*/
	public var animationExitOptions: UIView.AnimationOptions = DropDownList.animationExitOptions
...
extension DropDownList {
	...
	*/
    ...
    public func show(onTopOf window: UIWindow? = nil, beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
		if self == DropDownList.VisibleDropDown && DropDownList.VisibleDropDown?.isHidden == false { // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue when continuous taping on button - Kartik Patel - 2016-12-29
			return (true, 0)
		}

		if let visibleDropDown = DropDownList.VisibleDropDown {
			visibleDropDown.cancel()
		}

		willShowAction?()

		DropDownList.VisibleDropDown = self
	...
	/// Hides the drop down.
	public func hide() {
		if self == DropDownList.VisibleDropDown {
			/*
			...
			*/
			DropDownList.VisibleDropDown = nil
		}
